...
read_host_git_config() {
    ...
    if [[ -n "${worktree}" ]]; then
        git -C "${worktree}" config --get "${key}" 2>/dev/null || true
        return 0
    fi
...
write_git_identity_file() {
    ...
    git_user_name="${GIT_USER_NAME:-$(read_host_git_config user.name "${worktree}")}"
    git_user_email="${GIT_USER_EMAIL:-$(read_host_git_config user.email "${worktree}")}"
